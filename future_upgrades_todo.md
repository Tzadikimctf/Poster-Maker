...
## 🗺️ Phase 1: Poster Editor Modularity Upgrades
...
- `[ ]` **Preset Serialization & Load:**
  - Ensure all custom layers (coordinates, dimensions, text, fonts, colors) serialize cleanly into the preset JSON output.
  - Implement full restoration of custom layers when loading a user-uploaded preset JSON.

### 2. UI/UX & Interaction Upgrades
- `[ ]` **Sidebar-Canvas Syncing:**
  - Bind the "Advanced Drag & Drop" checkbox on the *DESIGN* tab to the *Standard/Layers* sidebar selector:
    - Checking the box triggers `setSidebarMode('advanced')`.
    - Unchecking the box triggers `setSidebarMode('standard')`.
- `[ ]` **Interactive Layer Tree Controls:**
  - Add lock (`🔒`) toggles to each layer in the Layers list to disable dragging and resizing.
  - Add hide (`👁️`) toggles in the list to skip rendering specific layers.
  - Add duplicate (`📄`) and delete (`🗑️`) buttons directly inside the sidebar tree item rows.
- `[ ]` **Undo / Redo Stack:**
...
- `[ ]` **Header & Footer Templating:**
  - Automatically replicate contact details (header) or page numbers (footer) on page 2 and beyond.
